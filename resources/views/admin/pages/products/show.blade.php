                        ...
                        <div class="col-lg-5 mb-4">
                                ...
                                <div class="carousel-inner" style="height:300px;">
                                    @foreach($product->images as $img)
                                        <div class="carousel-item {{ $loop->first ? 'active' : '' }}">
                                            <img src="{{ Storage::url($img->image_path) }}" class="d-block mx-auto"
                                                alt="{{ $product->name }}" style="max-height:300px; object-fit:contain;" />
                                        </div>
                                    @endforeach
                                </div>
                            ...
                            <ul class="list-group list-group-flush mt-3">
                                <li class="list-group-item">
                                    ...
                                    @php
                                        $statusLabels = [
                                            'published' => ['label' => 'Đã đăng', 'class' => 'success'],
                                            'draft' => ['label' => 'Nháp', 'class' => 'secondary'],
                                            'discontinued' => ['label' => 'Ngưng kinh doanh', 'class' => 'danger'],
                                        ];
                                        $status = $product->status ?? 'draft';
                                    @endphp

                                    <span class="badge bg-{{ $statusLabels[$status]['class'] ?? 'secondary' }}">
                                ...
                                <li class="list-group-item"><strong>Ngày tạo:</strong>
                                    {{ $product->created_at->format('d/m/Y') }}</li>
                            </ul>
                        </div>
                                                ...
                                                <tr>
                                                    <td>
                                                        @foreach($variant->attributeValues as $av)
                                                            <span class="badge bg-light text-dark me-1">{{ $av->attribute->name }}:
                                                                {{ $av->value }}</span>
                                                        @endforeach
                                                    </td>
