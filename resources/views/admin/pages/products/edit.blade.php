    ...
    <form action="{{ route('admin.products.update', $product->id) }}" method="POST" enctype="multipart/form-data" id="productForm">
        ...
        <div class="mb-4">
            ...
            <div id="current-thumbnail" class="mb-3">
                @if($thumb)
                <img src="{{ asset($thumb->image_path) }}"
        ...
        <div class="mb-4">
            ...
            <select name="product_type" id="product_type" class="form-select" required>
                <option value="simple" {{ old('product_type', $product->product_type) == 'simple' ? 'selected' : '' }}>Đơn</option>
                <option value="variable" {{ old('product_type', $product->product_type) == 'variable' ? 'selected' : '' }}>Biến thể</option>
            </select>
        </div>

        <!-- Khi chọn loại 'variable' -->
        <div id="variable-section" style="display: {{ old('product_type', $product->product_type) == 'variable' ? 'block' : 'none' }};">
            ...
            <div id="variable-result-section" style="display: {{ old('product_type', $product->product_type) == 'variable' && ($product->variants->count() || old('variants')) ? 'block' : 'none' }};">
                ...
                <div id="variable-list" class="space-y-3">
                    ...
                    <div class="row align-items-center mb-2">
                        <div class="col-md-4">
                            ...
                            <input type="hidden" name="variants[{{ $index }}][attribute_values][]" value="{{ $value->id }}">
                            @endforeach
                            @else
                            @foreach($variant['attribute_values'] as $valueId)
                            <input type="hidden" name="variants[{{ $index }}][attribute_values][]" value="{{ $valueId }}">
                            @endforeach
                            @endif
                        </div>
                        <div class="col-md-2">
                            <input type="number" name="variants[{{ $index }}][price]" class="form-control" placeholder="Giá"
                                value="{{ old("variants.$index.price", is_array($variant) ? ($variant['variant_price'] ?? '') : $variant->variant_price) }}">
                        </div>

                        <div class="col-md-2">
                            <input type="number" name="variants[{{ $index }}][quantity]" class="form-control" placeholder="SL"
                                value="{{ old("variants.$index.quantity", is_array($variant) ? ($variant['stock_quantity'] ?? '') : $variant->stock_quantity) }}">
                        </div>
        ...
        <div id="price-quantity-section" style="display: {{ old('product_type', $product->product_type) == 'simple' ? 'block' : 'none' }};">
            <div class="mb-4">
