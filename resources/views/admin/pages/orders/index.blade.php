                                ...
                                @forelse($orders as $index => $order)
                                                            <tr>
                                                                ...
                                                                <td class="text-start">
                                                                    ...
                                                                    <small>{{ $order->userAddress->ward ?? '' }},
                                                                        {{ $order->userAddress->district ?? '' }},
                                                                        {{ $order->userAddress->city ?? '' }}</small>
                                                                </td>
                                                                <td>{{ $order->shippingMethod->name ?? '---' }}</td>
                                                                <td class="text-start">
                                                                    <div>Tạm tính:
                                                                        <strong>{{ number_format($order->total_amount, 0, ',', '.') }}₫</strong></div>
                                                                    <div>Giảm giá: <strong
                                                                            class="text-danger">{{ number_format($order->discount_amount, 0, ',', '.') }}₫</strong>
                                                                    </div>
                                                                    <div>Phí ship:
                                                                        <strong>{{ number_format($order->shipping_fee, 0, ',', '.') }}₫</strong></div>
