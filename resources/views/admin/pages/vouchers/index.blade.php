                    ...
                    <form method="GET" action="{{ route('admin.vouchers.index') }}" class="row g-2 mb-3">
                        ...
                        <div class="col-md-2">
                            <select name="type" class="form-select">
                                ...
                                <option value="fixed" {{ request('type') == 'fixed' ? 'selected' : '' }}>VND (Cố định)
                                </option>
                            </select>
                        </div>
                        <div class="col-md-2">
                        ...
                        <div class="col-md-2">
                            <input type="date" name="expires_at" value="{{ request('expires_at') }}" class="form-control"
                                placeholder="Hết hạn">
                        </div>
                        <div class="col-md-3 d-flex justify-content-end">
                            <button type="submit" class="btn btn-secondary me-2">Lọc</button>
                            <a href="{{ route('admin.vouchers.index') }}" class="btn btn-outline-secondary">Đặt lại</a>
                        </div>
                    </form>
                                ...
                                @forelse($vouchers as $key => $voucher)
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            @else
                                                <span class="text-danger fw-bold">
                                                    {{ number_format($voucher->value, 0, ',', '.') }}đ</span>
                                            @endif
                                        </td>

                                        <td>{{ $voucher->max_discount_amount ? number_format($voucher->max_discount_amount) . 'Đ' : '-' }}
                                        </td>
                                        <td>{{ $voucher->min_order_value ? number_format($voucher->min_order_value) . 'Đ' : '-' }}
                                        </td>
                                        <td>{{ $voucher->max_usage ?? '-' }}</td>
                                        <td>{{ $voucher->expires_at ? \Carbon\Carbon::parse($voucher->expires_at)->format('d/m/Y') : '-' }}
                                        </td>
                                        <td>
                                        ...
                                        <td class="text-center">
                                            <form action="{{ route('admin.vouchers.toggle-status', $voucher->id) }}"
                                                method="POST" class="d-inline">
                                                @csrf
                                                @method('PATCH')
                                                <div class="form-check form-switch d-flex justify-content-center mb-0">
                                                    <input class="form-check-input" type="checkbox" name="is_active"
                                                        onchange="this.form.submit()" {{ $voucher->is_active ? 'checked' : '' }}>
                                                </div>
                                            </form>
                                        </td>
                                        <td>
